...
class GANimation(BaseModel):
    ...
    def _create_generator(self):
        return NetworksFactory.get_by_name(
            "generator_wasserstein_gan", c_dim=self._opt.cond_nc
        )

    def _create_discriminator(self):
        return NetworksFactory.get_by_name(
            "discriminator_wasserstein_gan", c_dim=self._opt.cond_nc
        )
    ...
    def _init_prefetch_inputs(self):
        self._input_real_img = self._Tensor(
            self._opt.batch_size, 3, self._opt.image_size, self._opt.image_size
        )
    ...
    def forward(self, keep_data_for_visuals=False, return_estimates=False):
        if not self._is_train:
            ...
            fake_imgs_masked = (
                fake_img_mask * real_img + (1 - fake_img_mask) * fake_imgs
            )
            ...
            rec_real_imgs = (
                rec_real_img_mask * fake_imgs_masked
            ...
            if return_estimates:
                ...
                fake_img_mask_max = fake_imgs_masked.view(
                    fake_img_mask.size(0), -1
                ).max(-1)[0]
                fake_img_mask_max = torch.unsqueeze(
                    torch.unsqueeze(torch.unsqueeze(fake_img_mask_max, -1), -1), -1
                )
                ...
                im_concat_img_batch = np.concatenate(
                    [
                        im_real_img_batch,
                        im_fake_imgs_masked_batch,
                        im_fake_img_mask_norm_batch,
                        im_fake_imgs_batch,
                    ],
                    1,
                )
    ...
    def _forward_G(self, keep_data_for_visuals):
        ...
        fake_imgs_masked = (
            fake_img_mask * self._real_img + (1 - fake_img_mask) * fake_imgs
        )

        # D(G(Ic1, c2)*M) masked
        (
            d_fake_desired_img_masked_prob,
            d_fake_desired_img_masked_cond,
        ) = self._D.forward(fake_imgs_masked)
        self._loss_g_masked_fake = (
            self._compute_loss_D(d_fake_desired_img_masked_prob, True)
            * self._opt.lambda_D_prob
        )
        self._loss_g_masked_cond = (
            self._criterion_D_cond(d_fake_desired_img_masked_cond, self._desired_cond)
            / self._B
            * self._opt.lambda_D_cond
        )

        # G(G(Ic1,c2), c1)
        rec_real_img_rgb, rec_real_img_mask = self._G.forward(
            fake_imgs_masked, self._real_cond
        )
        rec_real_img_mask = self._do_if_necessary_saturate_mask(
            rec_real_img_mask, saturate=self._opt.do_saturate_mask
        )
        ...
        self._loss_g_cyc = (
            self._criterion_cycle(rec_real_imgs, self._real_img) * self._opt.lambda_cyc
        )
        ...
        self._loss_g_mask_1_smooth = (
            self._compute_loss_smooth(fake_img_mask) * self._opt.lambda_mask_smooth
        )
        self._loss_g_mask_2_smooth = (
            self._compute_loss_smooth(rec_real_img_mask) * self._opt.lambda_mask_smooth
        )
    ...
    def _forward_D(self):
        ...
        self._loss_d_real = (
            self._compute_loss_D(d_real_img_prob, True) * self._opt.lambda_D_prob
        )
        self._loss_d_cond = (
            self._criterion_D_cond(d_real_img_cond, self._real_cond)
            / self._B
            * self._opt.lambda_D_cond
        )
        ...
        self._loss_d_fake = (
            self._compute_loss_D(d_fake_desired_img_prob, False)
            * self._opt.lambda_D_prob
        )
    ...
    def _gradinet_penalty_D(self, fake_imgs_masked):
        ...
        interpolated = Variable(
            alpha * self._real_img.data + (1 - alpha) * fake_imgs_masked.data,
            requires_grad=True,
        )
    ...
    def _compute_loss_smooth(self, mat):
        return torch.sum(torch.abs(mat[:, :, :, :-1] - mat[:, :, :, 1:])) + torch.sum(
    ...
    def get_current_visuals(self):
        ...
        visuals["1_input_img"] = plot_utils.plot_au(
            self._vis_real_img, self._vis_real_cond, title=title_input_img
        )
        visuals["2_fake_img"] = plot_utils.plot_au(
            self._vis_fake_img, self._vis_desired_cond
        )
        visuals["3_rec_real_img"] = plot_utils.plot_au(
            self._vis_rec_real_img, self._vis_real_cond
        )
    ...
    def _do_if_necessary_saturate_mask(self, m, saturate=False):
        return (
            torch.clamp(0.55 * torch.tanh(3 * (m - 0.5)) + 0.5, 0, 1) if saturate else m
        )
