...
class Train:
    ...
    def _train_epoch(self, i_epoch):
        ...
        for i_train_batch, train_batch in enumerate(self._dataset_train):
            ...
            # display flags
            do_visuals = (
                self._last_display_time is None
                or time.time() - self._last_display_time > self._opt.display_freq_s
            )
            do_print_terminal = (
                time.time() - self._last_print_time > self._opt.print_freq_s
                or do_visuals
            )
            ...
            train_generator = (
                (i_train_batch + 1) % self._opt.train_G_every_n_iterations == 0
            ) or do_visuals
            self._model.optimize_parameters(
                keep_data_for_visuals=do_visuals, train_generator=train_generator
            )
            ...
            if do_print_terminal:
                self._display_terminal(
                    iter_start_time, i_epoch, i_train_batch, do_visuals
                )
                self._last_print_time = time.time()
            ...
            # save model
            if (
                self._last_save_latest_time is None
                or time.time() - self._last_save_latest_time
                > self._opt.save_latest_freq_s
            ):
                print(
    ...
    def _display_terminal(self, iter_start_time, i_epoch, i_train_batch, visuals_flag):
        ...
        self._tb_visualizer.print_current_train_errors(
            i_epoch, i_train_batch, self._iters_per_epoch, errors, t, visuals_flag
        )
    ...
    def _display_visualizer_train(self, total_steps):
        ...
        self._tb_visualizer.plot_scalars(
            self._model.get_current_errors(), total_steps, is_train=True
        )
        self._tb_visualizer.plot_scalars(
            self._model.get_current_scalars(), total_steps, is_train=True
        )
